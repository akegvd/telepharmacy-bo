...
describe("TaskDetailContent", () => {
  ...
  it("renders the task once loaded", async () => {
    mockFetchTask.mockResolvedValue(makeRawTask({ id: "1", customerName: "Somchai P.", status: "new" }));
  ...
  it("shows the completed message with no advance action for a completed task", async () => {
    mockFetchTask.mockResolvedValue(makeRawTask({ id: "1", status: "completed" }));
  ...
  it("advances the task status when the button is clicked", async () => {
    ...
    mockFetchTask.mockResolvedValue(makeRawTask({ id: "1", status: "new" }));
