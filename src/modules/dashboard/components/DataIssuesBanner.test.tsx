...
describe('DataIssuesBanner', () => {
  it('renders nothing when there are no flagged issues or duplicates', () => {
    const { container } = render(<DataIssuesBanner tasks={[makeTask({ issues: [] })]} duplicateIds={[]} />);

    expect(container).toBeEmptyDOMElement();
  });

  it('uses singular wording for exactly one flagged record', () => {
    render(<DataIssuesBanner tasks={[makeTask({ issues: ['missing_name'] })]} duplicateIds={[]} />);
  ...
  it('uses plural wording for multiple flagged records', () => {
    render(
      <DataIssuesBanner
        tasks={[makeTask({ id: '1', issues: ['missing_name'] }), makeTask({ id: '2', issues: ['invalid_date'] })]}
        duplicateIds={[]}
      />
    );
